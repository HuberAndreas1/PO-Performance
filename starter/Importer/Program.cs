...
try
{
    ...
    if (!File.Exists(parsedArgs.LogFilePath))
    {
        Console.Error.WriteLine($"Error: File '{parsedArgs.LogFilePath}' not found.");
        return 1;
    }

    // Get the importer service from DI container
    var importer = host.Services.GetRequiredService<ICyberLiftImporter>();

    // Perform the import
    var importedCount = await importer.ImportAsync(parsedArgs.LogFilePath, parsedArgs.IsDryRun);
...
static void ConfigureServices(IServiceCollection services, IConfiguration configuration)
{
    ...
    services.AddScoped<ICyberLiftLogParser, CyberLiftLogParser>();
    services.AddScoped<IImportDatabaseWriter, ImportDatabaseWriter>();
    services.AddScoped<IBusinessLogic, BusinessLogic>();
    services.AddScoped<ICyberLiftImporter, CyberLiftImporter>();
}
